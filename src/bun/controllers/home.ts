...
import { getPlaylist, writeLogs } from "../db/index.ts";
...
async function loadHomeData(player: Player, pin: string[]): Promise<{ data: HomeData; complete?: boolean } | null> {
    ...
    const artistPins = newPins.filter((item: Pin) => item.type === MusicType.Artist);
    ...
    const [pinArtists, pinPlaylists, pinTracks, ytbNewTracks] = await Promise.all([
        Promise.all(artistPins.map((item: Pin) => player.youtubeDataAPI.fetchArtist(item.id, true))),
        Promise.all(playlistPins.map((item: Pin) => {
            if (item.source === MusicSource.Local) return getPlaylist(item.id, true);
            return player.youtubeDataAPI.fetchPlaylist(item.id, true);
        })),
        player.youtubeDataAPI.fetchTrack(trackPins.map(item => item.id)),
        player.youtubeDataAPI.getNewTracks(artistPins.map((item: Pin) => item.id).filter(item => item !== undefined)),
    ]);
