...
export default function Artist({ url }: { url: string }) {
    ...
    const refresh = async (source: string, id: string) => {
        ...
        const temp = (
            <>
                <Top name={artist.name} thumbnail={artist.thumbnail} duration={0} source={source} id={id} mode={"artist"} playlist={artist.tracks} />
                <List list={artist.tracks} source={source} id={id} mode="artist" />
            </>
        )
        setdom(temp)
    }
