...
export default function Sidebar({
    ...
    return (
        <div className="w-[30%] max-w-[250px] h-[100%] p-5 bg-slate-100 text-slate-800 dark:bg-black dark:text-white">
            ...
            <div className="playlists mt-5 cursor-default select-none h-[100%]">
                <div className="chose_sidebar flex flex-row justify-around">
                    {options.map(({ key, label }) => (
                        ...
                        >
                            <h2 className={`mb-2.5 text-sm font-bold uppercase tracking-wider text-gray-500 dark:text-neutral-400 select-none rounded-full ${selected === key ? "bg-slate-500 dark:bg-slate-300" : ""} px-2`}>
                                {label}
                            </h2>
                        </div>
                    ))}
