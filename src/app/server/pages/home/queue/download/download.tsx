...
export default function Download({ list }: { list: any[] }) {
    ...
    useEffect(() => {
        async function run() {
            ...
            const data = await fetch_data(Data.download, { format: format, links: links })
            // const data = await post("/download", { links: links, format: format });
            if (data.message == "OK") {
                setstatus(Status.downloading);
            }
        }
    ...
    return (
        <div>
            <span className="bg-green-400 hover:bg-green-300 h-[45px] w-[150px] flex flex-row items-center justify-between rounded-full"
                onClick={() => {
                    setdownload(true);
                }}
            >
                <span className="bg-slate-50 h-9 w-9 rounded-[50px] ml-[6px] flex flex-row items-center justify-center">
                    <FontAwesomeIcon icon={faDownload} className="text-slate-700 size-4" />
                </span>
