...
import { Mode, Music_options } from "./types.ts";
...
export default class Music {
    ...
    async search(link: string, where?: string): Promise<any> {
        let { source, mode, id } = this.convert_link(link);
        if (source === "youtube") {
            if (mode === "playlist") {
        ...
        // if link from spotify
            ...
            if (where === "youtube") {
                return await this.youtube.search_youtube_video(link);
            }
            else if (where === "spotify") {
                return await this.spotify.search_spotify_video(link);
            }
        }
    }
}