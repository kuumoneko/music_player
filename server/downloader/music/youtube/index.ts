...
export default class Youtube {
    ...
    async get_new_tracks(ids: string[]) {
        try {
            ...
            let this_artist: any = null,
                this_playlist: any = null;
            const tracks: string[] = [];

            for (const id of ids) {
                const artist_url = base_artist__url + `&id=${id}`;
                this_artist = this.getdata("artist", id);
                ...
                this_playlist = this.getdata("playlist", playlistId) ?? { ids: [] };
                ...
                if (video === null && playlist_etag !== new_tracks_etag) {
                    ...
                    if (this_new_tracks?.ids?.length === 0 || this_new_tracks?.ids === null) {
                        updated_items.push(...this_playlist.ids.slice(0, 6))
                    }
                    else {
                        for (const item of this_playlist.ids) {
                            if (this_new_tracks?.ids?.includes(item)) break;
                    ...
                    while (!done) {
                        let is_done = false;
                        if (this_new_tracks.ids?.length === 0 || this_new_tracks.ids === null) {
                            playlist_ids.push(...video.items.map((track: any) => { return track.snippet.resourceId.videoId }))
                            done = true;
                        }
                        else {
                            for (const item of video.items as Track[]) {
                                if (this_new_tracks?.ids?.findIndex((new_track: string) => { return new_track === item.track?.id }) !== -1) {
                                    is_done = true;
                                    break;
                                }
                    ...
                    const data_tracks = [... new Set([...this_playlist?.ids || [], ...playlist_ids])];
                    tracks.push(...playlist_ids.slice(0, 6));
                    this_playlist = {
                        etag: etag,
                        thumbnail: thumbnail,
                        name: name as string,
                    ...
                    tracks.push(...this_new_tracks.ids.slice(0, 6));
                }
                this.writedata("artist", id, this_artist);
                this.writedata("playlist", playlistId, this_playlist);
                this_artist = null;
                this_playlist = null;
            }
    ...
    async test_new_tracks(ids: string[]) {
        const new_tracks = this.getdata("new_tracks") ?? {};
