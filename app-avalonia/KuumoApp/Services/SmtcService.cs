...
public sealed class SmtcService : IDisposable
{
    ...
    private SystemMediaTransportControls? GetForWindow(IntPtr hwnd)
    {
        try
        {
    ...
    private interface ISystemMediaTransportControlsInterop
    {
        ...
        [PreserveSig] int GetForWindow(IntPtr appWindow, ref Guid riid, out IntPtr systemMediaTransportControls);
    }
    ...
    public void Update(SmtcUpdateDto data)
    {
        ...
        {
            AppLog.Write("smtc", "Update skipped: smtc or updater null");
            return;
        }
        try
        {
            ...
            _updater.Update();
            AppLog.Write("smtc", $"Update: title='{data.Title}' artist='{data.Artist}' playing={data.IsPlaying}");
            if (!_isPaused)
            {
    ...
    private async Task LoadThumbnailAsync(string? url)
    {
        ...
        {
            if (_pendingThumbnailUrl == url) _pendingThumbnailUrl = null;
        }
    }
