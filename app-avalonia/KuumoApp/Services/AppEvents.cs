...
public sealed class AppEvents
{
    ...
    public void RaiseRefetch()
    {
        ...
        {
            Dispatcher.UIThread.Post(() => RefetchRequested());
        }
    }
    ...
    public void OnEvent(string name, JsonElement data)
    {
        switch (name)
        {
            ...
            case "local-files-changed":
                if (LocalFilesChanged is not null) Dispatcher.UIThread.Post(() => LocalFilesChanged());
                break;
            case "smtc-update":
            ...
            case "dataChanged":
                if (DataChanged is not null)
                {
                    ...
                    if (key is not null)
                    {
                        Dispatcher.UIThread.Post(() => DataChanged(key));
                    }
                }
                break;
            case "open-app":
                if (OpenApp is not null) Dispatcher.UIThread.Post(() => OpenApp());
                break;
            case "app-exit":
                if (AppExit is not null) Dispatcher.UIThread.Post(() => AppExit());
                break;
            case "trackUnavailable":
                if (TrackUnavailable is not null) Dispatcher.UIThread.Post(() => TrackUnavailable());
                break;
            case "refetch":
                if (RefetchRequested is not null) Dispatcher.UIThread.Post(() => RefetchRequested());
                break;
        }
    }
