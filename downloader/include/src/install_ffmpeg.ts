...
async function unzip(path: string) {
    const child = spawn(`${path}\\include\\support\\7-Zip\\7z.exe e \"${path}\\temping\\ffmpeg.7z\" -o\"${path}\\temping\\ffmpeg"`, { stdio: "inherit", shell: true })
    ...
    child.on('close', async () => {
        ...
        for (const ffmpeg of paths) {
            await moveFile(ffmpeg, `${path}\\include\\support\\ffmpeg`)
        }
